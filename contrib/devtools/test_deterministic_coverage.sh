...
TEST_DANECOIN_BINARY="src/test/test_danecoin"
...
if [[ ! -e ${TEST_DANECOIN_BINARY} ]]; then
    echo "Error: Executable ${TEST_DANECOIN_BINARY} not found. Run \"./configure --enable-lcov\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_DANECOIN_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
