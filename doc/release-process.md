...
* Update `src/chainparams.cpp` chainTxData with statistics about the transaction count and rate. Use the output of the RPC `getchaintxstats`, see
  [this pull request](https://github.com/Danecoin/Danecoin/pull/12270) for an example. Reviewers can verify the results by running `getchaintxstats <window_block_count> <window_last_block_hash>` with the `window_block_count` and `window_last_block_hash` from your output.
...
### Danecoin maintainers/release engineers, suggestion for writing release notes
...
### Fetch and create inputs: (first time, or when dependency versions change)
    ...
    pushd ./gitian-builder
    mkdir -p inputs
    wget -P inputs https://danecoin.org/cfields/osslsigncode-Backports-to-1.7.1.patch
...
Create (and optionally verify) the signed Windows binaries:
    ...
    mv build/out/danecoin-*win32-setup.exe ../danecoin-${VERSION}-win32-setup.exe
    popd
...
- Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to the danecoin.org server
...
- Announce the release:
  ...
  - Danecoin Core announcements list https://danecoin.org/en/list/announcements/join/

  - danecoin.org blog post

  - Update title of #danecoin on Freenode IRC

  - Optionally twitter, reddit /r/Danecoin, ... but this will usually sort out itself

  - Notify BlueMatt so that he can start building [the PPAs](https://launchpad.net/~danecoin/+archive/ubuntu/danecoin)
