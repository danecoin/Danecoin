...
The Danecoin Core daemon can be configured to act as a trusted "border
router", implementing the danecoin wire protocol and relay, making
...
The ZeroMQ feature in Danecoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building danecoind:
...
For instance:
    ...
    $ danecoind -zmqpubhashtx=tcp://127.0.0.1:241233 \
               -zmqpubhashtx=tcp://192.168.1.2:241233 \
               -zmqpubrawtx=ipc:///tmp/danecoind.tx.raw \
...
These options can also be provided in danecoin.conf.
...
From the perspective of danecoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into danecoind directly. Furthermore, no information is
...
using. Danecoind appends an up-counting sequence number to each
