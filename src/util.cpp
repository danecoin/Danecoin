...
const char * const DANECOIN_CONF_FILENAME = "danecoin.conf";
const char * const DANECOIN_PID_FILENAME = "danecoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "danecoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Danecoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Danecoin
    // Mac: ~/Library/Application Support/Danecoin
    // Unix: ~/.danecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Danecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Danecoin";
#else
    // Unix
    return pathRet / ".danecoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", DANECOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", DANECOIN_PID_FILENAME)));
}
