...
const char * const DANECOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/danecoin/danecoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //danecoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", DANECOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Danecoin
    // macOS: ~/Library/Application Support/Danecoin
    // Unix-like: ~/.danecoin
#ifdef WIN32
    // Windows
