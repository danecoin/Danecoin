...
static void SetupCliArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", DANECOIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to danecoind)", false, OptionsCategory::OPTIONS);
    ...
    if (response.status == 0) {
        ...
        if (failedToGetAuthCookie) {
            throw std::runtime_error(strprintf(
                ...
                GetConfigFile(gArgs.GetArg("-conf", DANECOIN_CONF_FILENAME)).string().c_str()));
        } else {
