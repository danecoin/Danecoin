...
QString formatDanecoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("danecoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(DanecoinUnits::format(DanecoinUnits::DANE, info.amount, false, DanecoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", DANECOIN_CONF_FILENAME));
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("danecoin-%s.desktop", chain);
}
